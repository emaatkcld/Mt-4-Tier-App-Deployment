...
# Install AWSCLI
sudo apt-get install awscli -y #by default ubuntu does not come with aws cli installed

# Copy the Apache2 Proxy Config File From S3 (using the objet S3 URI)
sudo rm /etc/apache2/sites-available/000-default.conf
sudo aws s3 cp s3://rod-proxy-app-db-config1522/000-default.conf /etc/apache2/sites-available/

# Enable Apache2 Reverse Proxy (porbably the m ost important element of this script). These are the apache modules that do the most job
sudo a2enmod proxy 
